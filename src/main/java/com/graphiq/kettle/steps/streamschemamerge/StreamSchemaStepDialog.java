...
public class StreamSchemaStepDialog extends BaseStepDialog implements StepDialogInterface {
  ...
  /**
   * The constructor should simply invoke super() and save the incoming meta object to a local variable, so it can
  ...
  /**
   * This method is called by Spoon when the user opens the settings dialog of the step. It should open the dialog and
   * return only once the dialog has been closed by the user.
   * 
   * If the user confirms the dialog, the meta object (passed in the constructor) must be updated to reflect the new
   * step settings. The changed flag of the meta object must reflect whether the step configuration was changed by the
   * dialog.
   * 
   * If the user cancels the dialog, the meta object must not be updated, and its changed flag must remain unaltered.
   * 
   * The open() method must return the name of the step after the user has confirmed the dialog, or null if the user
   ...
   * 
   * @param inputSteps
   *          Names of the steps that are being merged together
   */
